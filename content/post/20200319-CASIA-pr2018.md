...
(2) 令两类集合分别为$w_1$和$w_2$, 各类样本均值分别为$m_1$和$m_2$，总的样本均值为$m$，两类样本概率分别为$P_1$和$P_2$，则
$$
\begin{aligned}
& S_b=\sum_{i=1}^2P_i(m_i-m)(m_i-m)^T \\
...
$$S_b=0, S_w=1$$
因此$J_5=1$. 由于$10>1$，所以应选择第一个特征进行分类任务。
